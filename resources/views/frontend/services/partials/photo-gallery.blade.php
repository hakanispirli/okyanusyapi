...
@if($service->gallery_images && count($service->gallery_images) > 0)
    <section class="py-12 md:py-20 bg-white" x-data="photoGallery" @keydown.window="handleKeydown($event)">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="text-center mb-12">
                <h2 class="text-2xl sm:text-3xl lg:text-4xl font-display font-bold text-corporate-900 mb-4">
                    {{ $service->gallery_title ?? 'Proje Galerisi' }}
                </h2>
                @if($service->gallery_description)
                        ...
                        @foreach($service->gallery_images as $index => $image)
                            <div class="flex-shrink-0 w-72 snap-center">
                                <div class="relative aspect-[4/3] rounded-2xl overflow-hidden cursor-pointer group" @click="openGallery({{ $index }})">
                                    <img
                                        src="{{ $image['url'] ?? $image }}"
                                        alt="{{ $image['alt'] ?? $service->name . ' - Galeri Resmi ' . ($index + 1) }}"
